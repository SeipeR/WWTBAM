...
//    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "ResultTableViewCell", for: indexPath) as? ResultTableViewCell {
            ...
            let result = GameSingleton.instance.results[indexPath.row]
            cell.configure(result: "Верных ответов: \(result.correctAnswersCount) из \(result.questionsCount) (\(result.percentage)%)", date: result.date.description)
